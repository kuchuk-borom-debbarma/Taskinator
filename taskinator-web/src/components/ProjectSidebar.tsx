...
import { LayoutGrid, Hash, Users2, Settings, LogOut, Hexagon } from 'lucide-react';
    ...
    <div className="h-full w-[72px] bg-[#050505] border-r border-white-[0.03] flex flex-col items-center py-6 gap-8 shrink-0 relative z-50">
      ...
      <nav className="flex-1 flex flex-col gap-4">
        ...
        <RailItem
          icon={<Users2 size={20} />}
          label="Teams"
          onClick={() => {/* navigate to global teams */}}
        />
      </nav>
      ...
      <div className="flex flex-col gap-4 mt-auto">
        ...
        <div className="relative group">
           ...
           <div className="absolute left-[calc(100%+12px)] bottom-0 px-3 py-2 bg-white text-background rounded-xl opacity-0 group-hover:opacity-100 transition-all pointer-events-none z-[100] shadow-2xl">
              <p className="text-[10px] font-bold whitespace-nowrap">{username}</p>
              <button 
                onClick={onLogout}
...
const RailItem: React.FC<{
  ...
  return (
    <div className="relative group">
      ...
      {/* Tooltip */}
      <div className="absolute left-[calc(100%+8px)] top-1/2 -translate-y-1/2 px-3 py-1.5 bg-white text-background text-[11px] font-bold rounded-lg opacity-0 group-hover:opacity-100 transition-all scale-95 group-hover:scale-100 pointer-events-none whitespace-nowrap z-[100] shadow-2xl">
        {label}
