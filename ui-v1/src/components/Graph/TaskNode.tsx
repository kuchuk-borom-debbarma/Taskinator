import React, { memo } from 'react';
import { Handle, Position } from '@xyflow/react';
import type { NodeProps } from '@xyflow/react';
import { Link, useParams } from '@tanstack/react-router';
import type { ProjectTask } from '../../api/types';

export type TaskNodeData = {
  task: ProjectTask;
  isFocused: boolean;
  direction: 'incoming' | 'outgoing' | 'both';
};

export const TaskNode: React.FC<NodeProps> = memo(({ data }: any) => {
  const { task, isFocused, direction } = data as TaskNodeData;
  const { projectId } = useParams({ strict: false }) as any;
  
  const statusColors: Record<string, string> = {
    TODO: 'bg-[#475569]',
    IN_PROGRESS: 'bg-[#3b82f6]',
    DONE: 'bg-[#10b981]',
  };

  const ringColor = isFocused 
    ? 'ring-[#3b82f6]' 
    : direction === 'incoming' 
      ? 'ring-[#ef4444]' 
      : 'ring-[#3b82f6]';

  return (
    <div className="relative group">
      <Handle type="target" position={Position.Left} className="opacity-0" />
      <Handle type="source" position={Position.Right} className="opacity-0" />

      <Link 
        to="/projects/$projectId/tasks/$taskId" 
        params={{ projectId: projectId || task.project?.id || '', taskId: task.id }}
        className={`
          flex flex-col items-center gap-2 p-1 transition-all duration-300
          ${isFocused ? 'scale-110' : 'hover:scale-105'}
        `}
      >
        <div className={`
          w-4 h-4 rounded-full border-2 border-white shadow-sm ring-4 ring-opacity-20
          ${statusColors[task.status] || 'bg-gray-400'}
          ${ringColor}
          ${isFocused ? 'ring-opacity-40' : 'group-hover:ring-opacity-40'}
        `} />

        <div className={`
          px-2.5 py-1 rounded-lg border border-white/10 bg-[#1a1f2e]/80 backdrop-blur-md shadow-lg
          max-w-[180px] transition-all duration-200
          ${isFocused ? 'border-[#3b82f6] shadow-[#3b82f6]/20' : 'group-hover:border-white/30'}
        `}>
          <p className={`
            text-[11px] truncate leading-tight transition-colors text-white
            ${isFocused ? 'font-black' : 'font-semibold text-white/80'}
          `}>
            {task.title}
          </p>
        </div>
      </Link>
    </div>
  );
});
