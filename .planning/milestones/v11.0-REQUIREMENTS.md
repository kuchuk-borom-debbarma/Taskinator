...
### Requirements:
* [x] **REQ-01**: Implement `src/modules/auto-action/contextEngine.ts` housing the core context resolution primitives.
* [x] **REQ-02**: Expose a `ContextResolverRegistry` class to allow registering scope-specific resolvers.
  * Must map scope name (e.g., `'TASK'`) to a `ContextResolver` definition.
  * Expose a singleton instance `contextResolverRegistry`.
* [x] **REQ-03**: Expose a standard, stateless `fetchContext(options)` function:
...
### Requirements:
* [x] **REQ-04**: Implement task context resolver in `src/modules/auto-action/scopes/task/context.ts`:
...
* [x] **REQ-05**: Register the task context resolver in the registry during `initTaskScope()`.

---

## 3. Entrypoint & Testing

* [x] **REQ-06**: Re-export `contextResolverRegistry` and `fetchContext` from the root `index.ts` of the `auto-action` module.
* [x] **REQ-07**: Implement comprehensive unit tests in `src/modules/auto-action/__tests__/contextEngine.test.ts` asserting:
