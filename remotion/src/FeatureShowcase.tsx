...
const AnimatedItem: React.FC<{ 
	...
	const x = interpolate(movement, [0, 1], [startPos.x, endPos.x]);
	const y = interpolate(movement, [0, 1], [startPos.y, endPos.y]);
...
export const FeatureShowcase: React.FC = () => {
	...
	return (
		<AbsoluteFill style={{ background: GRADIENTS.bg }}>
			...
			<Sequence from={fps * 7} durationInFrames={fps * 20}>
				<AbsoluteFill style={{ padding: '20px' }}>
					<div style={{
						...
						<div style={{ 
							...
							transform: `translateY(${interpolate(spring({ frame: segmentFrame, fps }), [0, 1], [-10, 0])}px)`
						}}>
						...
						<Sequence from={fps * 5.5}>
							<h3 style={{ position: 'absolute', top: 160, left: centerX - 55, fontSize: '10px', fontWeight: '800', color: COLORS.muted, letterSpacing: '1px', textTransform: 'uppercase', opacity: interpolate(segmentFrame, [fps * 5.5, fps * 5.8, fps * 8.5, fps * 9.0], [0, 1, 1, 0]) }}>
								Incoming Tasks
							</h3>
						</Sequence>
