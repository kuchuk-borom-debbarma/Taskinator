...
import { createTask, createChildTask, createUser, createProject } from './src/__tests__/helpers/factories.ts';
...
async function run() {
    ...
    const child = await createChildTask(project.id, user.id, parent, { title: 'Child' });
    console.log('Child ID:', child.id, 'Path:', child.materializedPath);

    const check = await db.selectFrom('project_task').selectAll().where('fk_project_id', '=', project.id as any).execute();
    console.log('ALL TASKS:', check.map(t => ({ id: t.id, path: t.materialized_path })));
