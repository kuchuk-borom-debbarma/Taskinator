...
export class AutomationListener {
    ...
    private async processBatch(events: TriggerPayload[]) {
        ...
        const teamIds = Array.from(new Set(events.map(e => e.newState.teamId).filter(Boolean)));
        ...
        console.log(`[Automation Module] Processing batch of ${events.length} events. Fetched rules for ${taskIds.length} tasks.`);
