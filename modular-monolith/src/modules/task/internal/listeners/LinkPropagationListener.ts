...
import {
    rebuildTerminalPathsQuery,
    getDownstreamTaskIdsQuery,
} from '../TaskQueries.ts';
    ...
    async init() {
        await eventBus.subscribe('task-relationship-propagation-group', {
            ...
            [KAFKA_EVENTS.TASK_LINK.CREATED]: async (data: {
                ...
                console.log(
                    `[LinkPropagator] Link created, starting propagation from terminal: ${toTaskId}`,
                );

                // 1. Re-bake paths for the immediate terminal
                await rebuildTerminalPathsQuery({
                    projectId,
                    terminalId: toTaskId,
                });
            ...
            [KAFKA_EVENTS.TASK_LINK.DELETED]: async (data: {
                ...
                const { projectId, toTaskId } = data;
                console.log(
                    `[LinkPropagator] Link deleted, re-baking sub-tree from terminal: ${toTaskId}`,
                );
            ...
            [KAFKA_EVENTS.TASK_LINK.PROPAGATE]: async (data: {
                ...
                console.log(
                    `[LinkPropagator] Propagating for descendants of ${taskId} (Offset: ${offset})`,
                );
                ...
                if (downstreamIds.length === 0) {
                    console.log(
                        `[LinkPropagator] Finished propagation chain for task ${taskId}`,
                    );
                    return;
                }
