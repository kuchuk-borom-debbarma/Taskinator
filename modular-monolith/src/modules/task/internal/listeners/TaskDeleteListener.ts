...
export class TaskDeleteListener {
    async init() {
        await eventBus.subscribe('task-recursive-cleanup-group', {
            [KAFKA_EVENTS.PROJECT_TASK.PARENT_DELETED]: async (
                data: ProjectTask,
            ) => {
                ...
                if (deletedIds.length > 0) {
                    await eventBus.publish(
                        KAFKA_EVENTS.PROJECT_TASK.CHILDREN_DELETED,
                        {
                ...
                if (hasMore) {
                    await eventBus.publish(
                        KAFKA_EVENTS.PROJECT_TASK.PARENT_DELETED,
                        {
                            key: id,
                            data,
                        },
                    ...
                    console.log(
                        `[Task Service] Child deletion loop completed for task: ${id}`,
                    );
                }
            },
        });
