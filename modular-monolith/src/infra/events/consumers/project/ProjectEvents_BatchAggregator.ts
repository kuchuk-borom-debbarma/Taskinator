    ...
    async init() {
        ...
        await eventBus.subscribe(
            ...
            'project-aggregator-group',
            {
                ...
                [EVENT_TYPES.PROJECT.DELETED]:
                    this.handleProjectBatch.bind(this),
                [EVENT_TYPES.PROJECT.MEMBERS_ADDED]:
                    this.handleProjectBatch.bind(this),
                [EVENT_TYPES.PROJECT.MEMBERS_REMOVED]:
                    this.handleProjectBatch.bind(this),
            },
            { batch: true },
        );
        ...
        await aggregatorService.processAggregatorBatch(
            ...
            (unprocessed) => {
                ...
                for (const event of chronologicallyOrderedEvents) {
                    ...
                    switch (event.type) {
                        case EVENT_TYPES.PROJECT.CREATED:
                            current.netBalance += 1;
                            break;
                        case EVENT_TYPES.PROJECT.DELETED:
                            current.netBalance -= 1;
                            break;
                        case EVENT_TYPES.PROJECT.MEMBERS_ADDED:
                            current.membershipBalance +=
                                event.data.addedUserIds?.length || 0;
                            break;
                        case EVENT_TYPES.PROJECT.MEMBERS_REMOVED: {
                ...
                for (const state of projectStates.values()) {
                    ...
                    if (state.removedUserIds.length > 0) {
                        const existing =
                            memberRemovals.get(state.projectId) || [];
