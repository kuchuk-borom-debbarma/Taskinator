...
import { getRedisSubscriber, INSTANCE_ID } from './index';
...
export const startRedisBridge = async () => {
    ...
    await redisSubscriber.subscribe(channel, (err, _count) => {
        ...
        logger.info(
            `[Redis Bridge] Subscribed to targeted local routing channel: ${channel}`,
        );
    });
    ...
    redisSubscriber.on('message', (chan, message) => {
        ...
        try {
            ...
            logger.error(
                '[Redis Bridge] Failed to process targeted message: %s',
                err,
            );
        }
    });
