...
describe('Project Deleted EDA Flow — Integration', () => {
    ...
    it('deletes all tasks when a project is deleted', async () => {
        ...
        const project = await createProject(owner.id, { name: 'Doomed Project' });
        ...
        await projectService.deleteProjects({ userId: owner.id, projectIds: [project.id] });
    ...
    it('deletes all project members when a project is deleted', async () => {
        const owner = await createUser();
        const project = await createProject(owner.id, { name: 'Members Project' });
