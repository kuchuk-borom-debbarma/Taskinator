...
describe('ConditionEvaluator — Unit Tests', () => {
    ...
    describe('EQUALS / NOT_EQUALS', () => {
        it('EQUALS matches exact value on newState', () => {
            ...
            expect(evaluateRuleGroup({}, { status: 'TODO' }, group)).toBe(
                false,
            );
        });
        ...
        it('NOT_EQUALS passes when value differs', () => {
            ...
            expect(evaluateRuleGroup({}, { status: 'DONE' }, group)).toBe(
                false,
            );
        });
    });
    ...
    describe('GREATER_THAN / LESS_THAN', () => {
        it('GREATER_THAN compares numeric values', () => {
            const group: RuleGroup = {
                match: 'ALL',
                conditions: [{ field: 'score', op: 'GREATER_THAN', value: 5 }],
            };
        ...
        it('LESS_THAN compares numeric values', () => {
            const group: RuleGroup = {
                match: 'ALL',
                conditions: [{ field: 'score', op: 'LESS_THAN', value: 5 }],
            };
    ...
    describe('IN / NOT_IN / CONTAINS', () => {
        it('IN matches when newVal is in the array', () => {
            const group: RuleGroup = {
                match: 'ALL',
                conditions: [
                    { field: 'status', op: 'IN', value: ['DONE', 'REVIEW'] },
                ],
            };
            expect(evaluateRuleGroup({}, { status: 'DONE' }, group)).toBe(true);
            expect(evaluateRuleGroup({}, { status: 'REVIEW' }, group)).toBe(
                true,
            );
            expect(evaluateRuleGroup({}, { status: 'TODO' }, group)).toBe(
                false,
        ...
        it('CONTAINS checks substring presence', () => {
            ...
            expect(
                evaluateRuleGroup({}, { title: 'This is urgent!' }, group),
            ).toBe(true);
            expect(evaluateRuleGroup({}, { title: 'Normal task' }, group)).toBe(
                false,
            );
        });
    });
    ...
    describe('HAS_CHANGED', () => {
        it('passes when old and new value differ', () => {
            const group: RuleGroup = {
                match: 'ALL',
                conditions: [{ field: 'priority', op: 'HAS_CHANGED' }],
            };
            expect(
                evaluateRuleGroup(
                    { priority: 'LOW' },
    ...
    describe('CHANGED_FROM', () => {
        it('passes only when the old value matches exactly', () => {
            const group: RuleGroup = {
                match: 'ALL',
                conditions: [
                    { field: 'status', op: 'CHANGED_FROM', value: 'TODO' },
                ],
            };
            expect(
                evaluateRuleGroup(
    ...
    describe('IS_NULL / IS_NOT_NULL', () => {
        it('IS_NULL passes for null and undefined', () => {
            const group: RuleGroup = {
                match: 'ALL',
                conditions: [{ field: 'memberId', op: 'IS_NULL' }],
            };
            expect(evaluateRuleGroup({}, { memberId: null }, group)).toBe(true);
            expect(evaluateRuleGroup({}, { memberId: undefined }, group)).toBe(
                true,
            );
            expect(evaluateRuleGroup({}, { memberId: 'user-1' }, group)).toBe(
                false,
            );
        });
        ...
        it('IS_NOT_NULL passes when value is present', () => {
            const group: RuleGroup = {
                match: 'ALL',
                conditions: [{ field: 'memberId', op: 'IS_NOT_NULL' }],
            };
            expect(evaluateRuleGroup({}, { memberId: 'user-1' }, group)).toBe(
                true,
            );
            expect(evaluateRuleGroup({}, { memberId: null }, group)).toBe(
                false,
            );
        });
    });
    ...
    describe('RuleGroup — ALL / ANY logic', () => {
        ...
        it('nested ALL inside ANY evaluates correctly', () => {
            ...
            expect(
                evaluateRuleGroup(
                    {},
                    { status: 'DONE', priority: 'LOW', memberId: null },
                    group,
                ),
            ).toBe(true);
            ...
            expect(
                evaluateRuleGroup(
                    {},
                    { status: 'TODO', priority: 'HIGH', memberId: 'u1' },
                    group,
                ),
            ).toBe(true);
            ...
            expect(
                evaluateRuleGroup(
                    {},
                    { status: 'TODO', priority: 'HIGH', memberId: null },
                    group,
                ),
            ).toBe(false);
        });
        ...
        it('nested ANY inside ALL evaluates correctly', () => {
            ...
            expect(
                evaluateRuleGroup(
                    { status: 'TODO' },
                    { status: 'DONE', priority: 'HIGH', memberId: 'u1' },
                    group,
                ),
            ).toBe(true);
            ...
            expect(
                evaluateRuleGroup(
                    { status: 'TODO' },
                    { status: 'DONE', priority: 'LOW', memberId: 'u1' },
                    group,
                ),
            ).toBe(false);
            ...
            expect(
                evaluateRuleGroup(
                    { status: 'DONE' },
                    { status: 'DONE', priority: 'HIGH', memberId: null },
                    group,
                ),
            ).toBe(false);
        });
    ...
    describe('Edge Cases', () => {
        ...
        it('unknown operator returns false gracefully (no throw)', () => {
            const group = {
                match: 'ALL' as const,
                conditions: [
                    { field: 'status', op: 'UNKNOWN_OP' as any, value: 'DONE' },
                ],
            };
            expect(() =>
                evaluateRuleGroup({}, { status: 'DONE' }, group),
            ).not.toThrow();
            expect(evaluateRuleGroup({}, { status: 'DONE' }, group)).toBe(
                false,
            );
        ...
        it('handles missing field on newState gracefully', () => {
            const group: RuleGroup = {
                match: 'ALL',
                conditions: [{ field: 'priority', op: 'IS_NULL' }],
            };
