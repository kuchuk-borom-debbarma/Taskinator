...
const { db } = (await import('../../../../../database/index.ts')) as any;
...
describe('ProjectQueries', () => {
  ...
  describe('insertProject', () => {
    it('should insert a project and return it', async () => {
      ...
      const mockExecute = (jest.fn() as any).mockResolvedValue([mockProject]);
      const mockReturningAll = jest.fn().mockReturnValue({ execute: mockExecute });
  ...
  describe('deleteProjects', () => {
    it('should delete projects for a specific user', async () => {
      ...
      const mockDeleted = [
        { id: 'p1', fk_user_id: 'user-1', name: 'P1', description: null, created_at: new Date() },
        { id: 'p2', fk_user_id: 'user-1', name: 'P2', description: null, created_at: new Date() },
      ];

      const mockExecute = (jest.fn() as any).mockResolvedValue(mockDeleted);
      const mockReturningAll = jest.fn().mockReturnValue({ execute: mockExecute });
      const mockWhere2 = jest.fn().mockReturnValue({ returningAll: mockReturningAll });
      ...
      expect(mockWhere1).toHaveBeenCalledWith('id', 'in', data.projectIds);
      expect(mockWhere2).toHaveBeenCalledWith('fk_user_id', '=', data.userId);
